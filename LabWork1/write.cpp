...
void write(std::ofstream &rotate, Bitmapinfo *bitmap, Fileheader *header) {
    ...
    rotate.write(reinterpret_cast<char *>(&bitmap->biXPelsPerMeter),
                 sizeof(bitmap->biXPelsPerMeter));
    rotate.write(reinterpret_cast<char *>(&bitmap->biYPelsPerMeter),
                 sizeof(bitmap->biYPelsPerMeter));
